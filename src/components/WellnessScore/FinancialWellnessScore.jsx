...
const FinancialWellnessScore = ({ transactions }) => {
  ...
  // Update the score whenever transactions change
  useEffect(() => {
    if (transactions && transactions.length > 0) {
      ...
      // Reset score when there are no transactions
      setScore(0);
      setBreakdown({
        budgetAdherence: 0,
        savingsRate: 0,
        expenseStability: 0,
        incomeStability: 0,
        expenseDiversification: 0
      });
    }
  }, [transactions]); // Dependency on transactions ensures updates

  const calculateScore = (transactions) => {
    // Sort transactions by date to ensure consistent calculations
    const sortedTransactions = [...transactions].sort((a, b) => 
      new Date(a.date) - new Date(b.date)
    );

    // Calculate monthly data with sorted transactions
    ...
    const expenseDiversification = calculateExpenseDiversification(sortedTransactions);
